...
{
    // Convert artbyte:// to artbyte:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("artbyte://"))
    {
        uri.replace(0, 12, "artbyte:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Artbyte.lnk";
}
...
{
    return GetAutostartDir() / "artbyte.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Artbyte-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  artbyte-qt [" + tr("command-line options") + "]                     " + "\n";
...
void SetBlackThemeQSS(QApplication& app)
{
    app.setStyleSheet("QWidget        { background: rgb(190,170,170); }"//this is the color of the entire background on the right side
                      ...
                      "QTabBar::tab:selected  { background: rgb(90,252,82); }" //this is for "secondary window selector" and shows "turned on option"
                      "QTabBar::tab:!selected { background: rgb(255,0,128); margin-top: 2px; }" //this is for "secondary window selector" and shows "turned off option"
                      "QTabWidget::pane { border: 1px solid rgb(190,0,0); }" //this is the border around secondary dialogue windows
                      "QToolButton    { background: rgb(190,0,0); color: rgb(0,0,0); border: none; border-left-color: rgb(190,0,0); border-left-style: solid; border-left-width: 6px; margin-top: 8px; margin-bottom: 8px; }" //the first one determines the color of buttons on the left; second one is menu text; the last one is the divider between buttons on horizontal line as well as the "turned off" buttons in left context menu
                      "QToolButton:checked { color: rgb(255,255,255); border: none; border-left-color: rgb(90,252,82); border-left-style: solid; border-left-width: 6px; }" //the first one is text color... the second is button color to the left (pressed state)
                      ...
                      "QTreeView::item { background: rgb(190,0,0); color: rgb(212,213,213); }"
                      "QTreeView::item:selected { background-color: rgb(48,140,198); }"
                      "QTableView     { background: rgb(66,71,78); color: rgb(255,255,128); gridline-color: rgb(157,160,165); }" //Middle one determines receive address directory text color
                      "QHeaderView::section { background: rgb(190,0,0); color: rgb(255,255,255); }"
                      "QToolBar       { background: rgb(190,0,0); border: none; }");
}

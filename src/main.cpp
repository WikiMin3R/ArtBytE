...
// Copyright (c) 2018-2022 InfiniLooP developers / EWMCI, LLC.
// Copytight (c) 2020-2022 ArtBytE developers
...
#if defined(NDEBUG)
# error "Artbyte cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Artbyte Signed Message:\n";
...
bool CBlock::AcceptBlock()
{
    ...
    if (IsProtocolV2(nHeight) && nVersion < 8)
...
{
    string strBlockFn = strprintf("aby%04u.dat", nFile);
...
bool LoadExternalBlockFile(FILE* fileIn)
{
    ...
    {
        try {
            CAutoFile abydat(fileIn, SER_DISK, CLIENT_VERSION);
            unsigned int nPos = 0;
            while (nPos != (unsigned int)-1 && abydat.good())
            {
                boost::this_thread::interruption_point();
                unsigned char pchData[65536];
                do {
                    fseek(abydat, nPos, SEEK_SET);
                    int nRead = fread(pchData, 1, sizeof(pchData), abydat);
                    if (nRead <= 8)
                    {
                        nPos = (unsigned int)-1;
                        break;
